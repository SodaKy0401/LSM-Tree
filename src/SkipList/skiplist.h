...
public:
  ...
  void put(const std::string &key, const std::string &value,uint64_t tranc_id); // 插入或更新键值对
  
  // 查找键对应的值
  // 事务 id 为0 表示没有开启事务
  // 否则只能查找事务 id 小于等于 tranc_id 的值
  // 返回值: 如果找到，返回 value 和 tranc_id，否则返回空
  SkipListIterator get(const std::string &key,uint64_t tranc_id); // 查找键对应的值

  // !!! 这里的 remove 是跳表本身真实的 remove,  lsm 应该使用 put 空值表示删除
  void remove(const std::string &key); // 删除键值对

  std::vector<std::tuple<std::string, std::string,uint64_t>> flush(); // 将跳表数据刷出，返回有序键值对列表
